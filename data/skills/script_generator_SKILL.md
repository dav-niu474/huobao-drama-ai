...
> **注意：本技能指南中的工具调用部分（`get_planData` / `get_novel_events` / `get_novel_text` / `get_script_content` 等）暂未实现。**
> 上下文（故事骨架 / 改编策略 / 本集相关章节原文 / 上一集剧本）已通过用户 prompt 直接提供，无需调用工具。
> 请直接使用 prompt 中提供的上下文信息，按以下流程生成剧本。
...
> 上述工具当前未注册到本 Agent，仅作流程示意。实际使用时，上下文已由调用方通过 prompt 注入，请直接引用 prompt 中提供的内容。

## 执行流程

1. 上下文（故事骨架 / 改编策略 / 本集相关章节原文 / 上一集剧本）已由调用方通过 prompt 直接提供，**无需调用工具读取**；若 prompt 中已有"上一集剧本"段落，请直接引用以衔接剧情与角色状态。
