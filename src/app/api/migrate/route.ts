...
//
// Covers all 36 models in the Prisma schema (v2).
...
const MIGRATIONS: { table: string; sql: string }[] = [
  ...
  // ==========================================================
  // SECTION 3: V2 New tables — Season, ArtStyle, Clue
  // ==========================================================

  // ---- Season (季管理) — must exist before Episode.seasonId FK ----
  {
    table: 'Season',
    sql: `CREATE TABLE IF NOT EXISTS "Season" (
      ...
      CONSTRAINT "Season_dramaId_fkey" FOREIGN KEY ("dramaId") REFERENCES "Drama"("id") ON DELETE CASCADE ON UPDATE CASCADE
    );`,
  },
  {
    table: 'Season_dramaId_idx',
    sql: `CREATE INDEX IF NOT EXISTS "Season_dramaId_idx" ON "Season"("dramaId");`,
  },

  // ---- ArtStyle (画风包) ----
  {
    table: 'ArtStyle',
    sql: `CREATE TABLE IF NOT EXISTS "ArtStyle" (
      "id" TEXT NOT NULL,
      "key" TEXT NOT NULL,
      "name" TEXT NOT NULL,
      "category" TEXT NOT NULL DEFAULT '2D',
      "description" TEXT,
      "prefixMd" TEXT,
      "styleMeta" TEXT,
      "previewUrl" TEXT,
      ...
      CONSTRAINT "ArtStyle_key_key" UNIQUE ("key")
    );`,
  },
  ...
  {
    table: 'Clue',
    sql: `CREATE TABLE IF NOT EXISTS "Clue" (
      "id" TEXT NOT NULL,
      "dramaId" TEXT NOT NULL,
      "type" TEXT NOT NULL,
      "name" TEXT NOT NULL,
      ...
      CONSTRAINT "Clue_dramaId_fkey" FOREIGN KEY ("dramaId") REFERENCES "Drama"("id") ON DELETE CASCADE ON UPDATE CASCADE
    );`,
  },
  {
    table: 'Clue_dramaId_idx',
    sql: `CREATE INDEX IF NOT EXISTS "Clue_dramaId_idx" ON "Clue"("dramaId");`,
  },
  ...
  // ---- Drama: columns that existed before v2 (from main branch) ----
  {
  ...
  // ---- Drama: v2 new columns (商业协商 + 流程状态) ----
  {
    table: 'Drama_coverage',
    sql: addColumn('Drama', 'coverage', 'TEXT'),
  },
  {
    table: 'Drama_episodeFormat',
    sql: addColumn('Drama', 'episodeFormat', 'TEXT'),
  },
  {
    table: 'Drama_aspectRatio',
    sql: addColumn('Drama', 'aspectRatio', `TEXT DEFAULT '9:16'`),
  },
  {
    table: 'Drama_genreTone',
    sql: addColumn('Drama', 'genreTone', 'TEXT'),
  },
  {
    table: 'Drama_paywallConfig',
    sql: addColumn('Drama', 'paywallConfig', 'TEXT'),
  },
  {
    table: 'Drama_targetPlatform',
    sql: addColumn('Drama', 'targetPlatform', `TEXT DEFAULT 'douyin'`),
  },
  {
    table: 'Drama_budgetConstraints',
    sql: addColumn('Drama', 'budgetConstraints', 'TEXT'),
  },
  {
    table: 'Drama_novelAnalysis',
    sql: addColumn('Drama', 'novelAnalysis', 'TEXT'),
  },
  {
    table: 'Drama_showPlanLocked',
    sql: addColumn('Drama', 'showPlanLocked', 'BOOLEAN NOT NULL DEFAULT false'),
  },
  {
    table: 'Drama_scriptGenerationStatus',
    sql: addColumn('Drama', 'scriptGenerationStatus', `TEXT DEFAULT 'pending'`),
  },
  {
    table: 'Drama_assetExtractionStatus',
    sql: addColumn('Drama', 'assetExtractionStatus', `TEXT DEFAULT 'pending'`),
  },
  {
    table: 'Drama_currentPhase',
    sql: addColumn('Drama', 'currentPhase', `TEXT DEFAULT 'novel_upload'`),
  },

  // ---- Episode: columns that existed before v2 ----
  {
  ...
  // ---- Episode: v2 new columns ----
  {
    table: 'Episode_seasonId',
    sql: addColumn('Episode', 'seasonId', 'TEXT'),
  },
  {
    table: 'Episode_seasonId_fkey',
    sql: addFkConstraint('Episode_seasonId_fkey', 'Episode', 'seasonId', 'Season', 'id', 'SET NULL', 'CASCADE'),
  },
  {
    table: 'Episode_seasonId_idx',
    sql: `CREATE INDEX IF NOT EXISTS "Episode_seasonId_idx" ON "Episode"("seasonId");`,
  },

  // ---- Character: columns that existed before v2 ----
  {
  ...
  // ---- Character: v2 new columns (角色圣经 + 一致性) ----
  {
    table: 'Character_identityAnchors',
    sql: addColumn('Character', 'identityAnchors', 'TEXT'),
  },
  {
    table: 'Character_weightTier',
    sql: addColumn('Character', 'weightTier', `TEXT DEFAULT 'C'`),
  },
  {
    table: 'Character_generationOrder',
    sql: addColumn('Character', 'generationOrder', 'INTEGER NOT NULL DEFAULT 0'),
  },
  {
    table: 'Character_threeViewsUrl',
    sql: addColumn('Character', 'threeViewsUrl', 'TEXT'),
  },
  {
    table: 'Character_wardrobeUrls',
    sql: addColumn('Character', 'wardrobeUrls', 'TEXT'),
  },
  {
    table: 'Character_assetId_idx',
    sql: `CREATE INDEX IF NOT EXISTS "Character_assetId_idx" ON "Character"("assetId");`,
  },
  ...
  {
    table: 'Scene_assetId_idx',
    sql: `CREATE INDEX IF NOT EXISTS "Scene_assetId_idx" ON "Scene"("assetId");`,
  },

  // ---- Storyboard: columns that existed before v2 ----
  {
  ...
  // ---- Storyboard: v2 new columns (关键帧 + 候选) ----
  {
    table: 'Storyboard_generationMode',
    sql: addColumn('Storyboard', 'generationMode', `TEXT DEFAULT 'image2video'`),
  },
  {
    table: 'Storyboard_gridImageUrl',
    sql: addColumn('Storyboard', 'gridImageUrl', 'TEXT'),
  },
  {
    table: 'Storyboard_gridLayout',
    sql: addColumn('Storyboard', 'gridLayout', 'TEXT'),
  },
  {
    table: 'Storyboard_startFrameImageUrl',
    sql: addColumn('Storyboard', 'startFrameImageUrl', 'TEXT'),
  },
  {
    table: 'Storyboard_endFrameImageUrl',
    sql: addColumn('Storyboard', 'endFrameImageUrl', 'TEXT'),
  },
  {
    table: 'Storyboard_candidateUrls',
    sql: addColumn('Storyboard', 'candidateUrls', 'TEXT'),
  },
  {
    table: 'Storyboard_selectedCandidateIndex',
    sql: addColumn('Storyboard', 'selectedCandidateIndex', 'INTEGER'),
  },
  ...
  {
    table: 'Prop_assetId_idx',
    sql: `CREATE INDEX IF NOT EXISTS "Prop_assetId_idx" ON "Prop"("assetId");`,
  },
  ...
  // ==========================================================
  // SECTION 5: Collaboration & other tables
  // ==========================================================
...
// Required tables (from Prisma schema) — all 36 models
...
const REQUIRED_COLUMNS: Record<string, string[]> = {
  'Drama': ['novelSource', 'novelParsed', 'artStyle', 'assetStatus', 'defaultLockedConfig', 'styleTemplate', 'seriesId',
            'coverage', 'episodeFormat', 'aspectRatio', 'genreTone', 'paywallConfig', 'targetPlatform', 'budgetConstraints',
            'novelAnalysis', 'showPlanLocked', 'scriptGenerationStatus', 'assetExtractionStatus', 'currentPhase'],
  'Episode': ['sourceChapterIds', 'globalAssetsImported', 'lockedConfig', 'videoUrl', 'duration', 'seasonId'],
  'Character': ['assetId', 'styleLock', 'lockedReferenceImage', 'visualFingerprint', 'episodeIds',
                'identityAnchors', 'weightTier', 'generationOrder', 'threeViewsUrl', 'wardrobeUrls'],
  ...
  'Storyboard': ['atmosphere', 'firstFrameUrl', 'lastFrameUrl', 'composedUrl', 'bgmPrompt', 'soundEffect', 'referenceImages',
                 'generationMode', 'gridImageUrl', 'gridLayout', 'startFrameImageUrl', 'endFrameImageUrl', 'candidateUrls', 'selectedCandidateIndex'],
  'Prop': ['assetId'],
