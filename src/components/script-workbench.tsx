...
export function ScriptWorkbench() {
  ...
  const handleReparse = async () => {
    ...
    try {
      ...
      toast({ title: `重新解析完成，共 ${data.chapterCount} 章` })
                ...
                <div className="p-4 max-w-4xl mx-auto">
                  {selectedChapterIdx !== null && chapters[selectedChapterIdx] ? (
                    <div className="space-y-3">
                      <div className="flex items-center justify-between">
                        ...
                        <div className="flex items-center gap-1.5">
                          <Button
                            ...
                            onClick={() => {
                              const newIdx = selectedChapterIdx - 1
                              setSelectedChapterIdx(newIdx)
                            }}
                          >
                      ...
                      <div className="prose prose-sm dark:prose-invert max-w-none">
                        <pre className="whitespace-pre-wrap text-sm leading-relaxed bg-muted/30 rounded-lg p-4 border border-border/50 max-h-[calc(100vh-200px)] overflow-y-auto">
                          {chapters[selectedChapterIdx].content}
                        </pre>
                      </div>
                    </div>
                  ) : chapters.length > 0 ? (
                    <EmptyState
                      ...
                      title="章节原文"
                      description="点击左侧章节列表查看原文内容，可对照骨架和策略进行创作"
                    />
                  ) : (
                    <EmptyState
                      icon={<BookOpen className="size-10 text-amber-500/40" />}
                      title="章节原文"
                      description="上传小说文件后，可在此查看各章节的原文内容"
                    />
                  )}
                </div>
              </ScrollArea>
